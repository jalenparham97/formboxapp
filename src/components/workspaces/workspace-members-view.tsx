...
import {
  type MemberWithUserAndWorkspace,
  WorkspaceRoles,
...
import {
  ...
  useWorkspaceById,
  useWorkspaceInvites,
  useWorkspaceMemberDeleteMutation,
  useWorkspaceMembers,
...
export function WorkspaceMembersView({ workspaceId }: Props) {
  ...
  const deleteInviteMutation = api.workspace.deleteInvite.useMutation({
    onSuccess: () => {},
    onSettled: async () => {
      apiUtils.workspace.getInvites.invalidate({ id: workspaceId });
    },
  });
  ...
  function updateTabState(tab: "members" | "invites") {
    return router.push(`/workspaces/${workspaceId}/members?tab=${tab}`);
  }
  return (
    <div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-gray-600">
              Teammates or collaborators that have access to this workspace and
              it&apos;s forms.
            </p>
          </div>
          <div>
                    ...
                    {isEmpty(invites) && (
                      <div className="mx-auto mb-5 mt-12 w-full lg:max-w-2xl">
                        <EmptyState
                          ...
                          subtitle="Invite a teammate to collaborate on this workspace with."
                          icon={
                            <IconMailForward
                              size={50}
...
interface MemberProps {
  member: MemberWithUserAndWorkspace;
  index: number;
}
...
function MemberCard({ member, index }: MemberProps) {
  ...
  const deleteMutation = useWorkspaceMemberDeleteMutation(
    member?.workspace?.id,
  );
  ...
  const isMemberRole = member.role === WorkspaceRoles.MEMBER;
  const isOwnerRole = member.role === WorkspaceRoles.OWNER;
